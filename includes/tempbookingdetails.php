<?php
	Class TempBookingDetails {
		...
		protected static $table_name="temp_booking_details";
		
		protected static $db_fields=array('id' ,'customer_id' ,'service_point_id' ,'product_category_id' ,'product_name' ,'product_description' ,'packet_size' ,'packet_weight' ,'purchase_invoice', 'packet_qty', 'pick_up_time', 'receive_amount', 'receiver_name' ,'receiver_phone' ,'receiver_email' ,'receiver_address' ,'booking_status' ,'entry_by' ,'entry_date' );
		...
		public function reset_booking($user_id){
		global $database;		
		$sql="DELETE FROM ".self::$table_name;
		$sql.=" WHERE entry_by=".$user_id." AND booking_status='Request' ";
		$sql.=" LIMIT 1 ";
		$database->query($sql);
		return ($database->affected_rows()==1) ? true : false; 
		}		
		...
		global $database;		
		$sql="DELETE FROM ".self::$table_name;
		$sql.=" WHERE entry_by=".$database->escape_value($this->entry_by)." && `booking_status`='Request' ";
		$database->query($sql);
		return ($database->affected_rows()==1) ? true : false; 
		}
